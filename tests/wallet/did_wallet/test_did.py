...
class TestDIDWallet:
    ...
    async def test_creation_from_coin_spend(
        ...
        async with wallet_node_0.wallet_state_manager.lock:
            did_wallet_0 = await DIDWallet.create_new_did_wallet(
    ...
    async def test_creation_from_backup_file(
        self, self_hostname: str, three_wallet_nodes: OldSimulatorsAndWallets, trusted: bool
    ) -> None:
        ...
        async with wallet_node_1.wallet_state_manager.lock:
            did_wallet_1 = await DIDWallet.create_new_did_wallet(
    ...
    async def test_did_recovery_with_multiple_backup_dids(
        ...
        async with wallet_node.wallet_state_manager.lock:
            did_wallet = await DIDWallet.create_new_did_wallet(wallet_node.wallet_state_manager, wallet, uint64(101))
        ...
        async with wallet_node_2.wallet_state_manager.lock:
            did_wallet_2 = await DIDWallet.create_new_did_wallet(
        ...
        async with wallet_node_2.wallet_state_manager.lock:
            did_wallet_3 = await DIDWallet.create_new_did_wallet(
        ...
        assert new_ph is not None
        message_spend_bundle, attest1 = await did_wallet.create_attestment(coin_id, new_ph, pubkey)
        spend_bundle_list = await wallet_node.wallet_state_manager.tx_store.get_unconfirmed_for_wallet(did_wallet.id())

        spend_bundle = spend_bundle_list[0].spend_bundle
        assert spend_bundle is not None
        await time_out_assert_not_none(5, full_node_api.full_node.mempool_manager.get_spendbundle, spend_bundle.name())
        message_spend_bundle2, attest2 = await did_wallet_2.create_attestment(coin_id, new_ph, pubkey)
        spend_bundle_list = await wallet_node_2.wallet_state_manager.tx_store.get_unconfirmed_for_wallet(
            did_wallet_2.id()
        )
        ...
        for wallet_to_check in [did_wallet, did_wallet_2, did_wallet_3, did_wallet_4]:
            assert wallet.wallet_state_manager.wallets[wallet.id()] == wallet_to_check
    ...
    async def test_did_find_lost_did(
        ...
        # TODO: Ask Kyle is something is to be done here about this cast
        did_wallet = cast(DIDWallet, wallet_node.wallet_state_manager.wallets[did_wallets[0].id])
        await time_out_assert(15, did_wallet.get_confirmed_balance, 101)
        await time_out_assert(15, did_wallet.get_unconfirmed_balance, 101)
        # Spend DID
    ...
    async def test_did_attest_after_recovery(
        ...
        for wallet_to_check in [did_wallet, did_wallet_3, did_wallet_4]:
            assert wallet.wallet_state_manager.wallets[wallet.id()] == wallet_to_check
    ...
    async def test_did_transfer(
        self, self_hostname: str, two_wallet_nodes: OldSimulatorsAndWallets, with_recovery: bool, trusted: bool
    ) -> None:
        fee = uint64(1000)
        full_nodes, wallets, _ = two_wallet_nodes
        full_node_api = full_nodes[0]
        ...
        did_wallet_2 = wallet_node_2.wallet_state_manager.wallets[did_wallets[0].id]
    ...
    async def test_update_recovery_list(
        ...
        [full_node_api], [(wallet_node, server_2)], _ = simulator_and_wallet
        server_1 = full_node_api.server
        wallet = wallet_node.wallet_state_manager.main_wallet
        ph = await wallet.get_new_puzzlehash()
    ...
    @pytest.mark.parametrize("trusted", [True, False])
    @pytest.mark.anyio
    async def test_get_info(self, self_hostname: str, two_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
        fee = uint64(1000)
        full_nodes, wallets, _ = two_wallet_nodes
        full_node_api = full_nodes[0]
        server_1 = full_node_api.server
        ...
        assert await did_wallet_1.get_confirmed_balance() == uint128(did_amount)
        assert await did_wallet_1.get_unconfirmed_balance() == uint128(did_amount)
        assert did_wallet_1.did_info.current_inner is not None
        assert did_wallet_1.did_info.origin_coin is not None
        origin_coin_id = did_wallet_1.did_info.origin_coin.name()
        response = await api_0.did_get_info({"coin_id": origin_coin_id.hex()})
        assert response["did_id"] == encode_puzzle_hash(origin_coin_id, AddressType.DID.value)
        assert response["launcher_id"] == origin_coin_id.hex()
        assert response["full_puzzle"] == create_singleton_puzzle(did_wallet_1.did_info.current_inner, origin_coin_id)
        ...
        assert await wallet1.get_confirmed_balance() == uint128(odd_amount)
    ...
    async def test_create_did_with_recovery_list(
        ...
        recovery_list = [bytes32.from_hexstr("00" * 32)]
        async with wallet_node_0.wallet_state_manager.lock:
            did_wallet_0 = await DIDWallet.create_new_did_wallet(
    ...
    async def test_did_resync(
        ...
        full_node_server_port = uint16(full_node_server._port)
        await wallet_server_1.start_client(PeerInfo(self_hostname, full_node_server_port), None)
        await wallet_server_2.start_client(PeerInfo(self_hostname, full_node_server_port), None)
        await full_node_api.farm_blocks_to_wallet(1, wallet)
